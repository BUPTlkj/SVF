...
for t in range(epoch):
    ...
    if a > min_acc:
        folder = 'save_model_FNN'
        if not os.path.exists(folder):
            os.mkdir('save_model_FNN')

        min_acc = a
        print('save best model original')

        torch.save(model.state_dict(), 'save_model_FNN/best_model_FNN.pth')

        # Specify a dummy input to the model
        dummy_input = torch.randn(1, 3, 8, 8, device='cpu')

        # Save the model as an ONNX file
        torch.onnx.export(model, dummy_input, 'save_model_FNN/best_model_FNN.onnx', input_names=['input'], output_names=['output'])
