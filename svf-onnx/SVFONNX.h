...
/// Neural Net struct
using SVFNeuralNet = std::variant<ConstantNodeInfo, BasicNodeInfo, ParsedGEMMParams, ConvNodeInfo, ReluNodeInfo, MaxPoolNodeInfo>;
...
public:
    ...
    std::map<std::string, std::string> callPythonFunction(const std::string& address, const std::string& functionName);
    ...
    std::map<std::string, std::pair<std::pair<int, int>, std::pair<int, int>>> parseConvItems(const std::vector<std::string>& items);
    std::map<std::string, std::pair<std::pair<int, int>, std::pair<std::pair<int, int>, std::pair<int, int>>>> parseMaxPoolItems(const std::vector<std::string>& items);
    ...
    Eigen::MatrixXd restoreGEMMWeightToMatrix(const std::string& dimensions, const std::string& values);
    Eigen::VectorXd restoreGEMMBiasMatrixFromStrings(const std::string& dimensionStr, const std::string& valuesStr);

    /// Conv Restore
    /// Add parse node
    ConvParams ConvparseAndFormat(const std::string& input);
    std::vector<SVF::FilterSubNode> parse_filters(const std::string& s, unsigned num_filters, unsigned kernel_height, unsigned kernel_width, unsigned kernel_depth);
