...
///// Parse function to extract integers from a given string and return their vectors
//
//
...
//    };
//
//    void DFS(std::set<const SVF::NeuronNode *> &visited, std::vector<const SVF::NeuronNode *> &path, const SVF::NeuronNode *src, const SVF::NeuronNode *dst){
//        visited.insert(src); // mark the current node as visited
//        path.push_back(src); // push the current node onto the path
//
//        if (src == dst) {
//            printPath(path);
//        }
...
//        for (const SVF::NeuronEdge *edge : src->getOutEdges()) {
//            const SVF::NeuronNode *neighbor = edge->getDstNode();
//            //            const SVF::NeuronNode *neighbor = edge->getDst();
//            if (visited.count(neighbor) == 0) { // the neighbor is unvisited
//                DFS(visited, path, neighbor, dst);
//            }
//        }
//
//        visited.erase(src);
//        path.pop_back(); // Backtrack by removing the current node from the path
//    }
//
...
//    /// init edges
//    std::vector<std::unique_ptr<SVF::NeuronEdge>> edges;
//
//    /// init Graph
...
//        std::cout << "ConstantNode: Name = " << node.name << std::endl;
...
//        std::cout << "BasicNodeInfo: Name = " << node.name<< std::endl;
...
//        std::cout << "GEMM: Name = " << node.gemmName << std::endl;
...
//        std::cout << "ConvNodeInfo: Name = " << node.name <<"  pads:"<<node.pads.first<< std::endl;
//        auto id = getNodeID(node.name);
//        OrderedNodeName.push_back(node.name);
...
//        std::cout << "ReluNodeInfo: Name = " << node.name << std::endl;
//        auto id = getNodeID(node.name);
//        OrderedNodeName.push_back(node.name);
...
//        std::cout << "maxPoolNodeInfo: Name = " << node.name <<"  pads:"<<node.pads.first<< std::endl;
//        auto id = getNodeID(node.name);
//        OrderedNodeName.push_back(node.name);
...
//    }
//
//    SVF::NeuronNode* getNodeInstanceByName(const std::string& name) const {
...
////            g->addNeuronNode(currentName);
//
//            SVF::NeuronNode* currentNode = getNodeInstanceByName(currentName);
//            SVF::NeuronNode* nextNode = getNodeInstanceByName(nextName);
//
//            if (currentNode && nextNode) {
//                // Ensure edge is created as a unique_ptr<SVF::Direct2NeuronEdge>
//                auto edge = std::make_unique<SVF::NeuronEdge>(currentNode, nextNode, 0);
//                edges.push_back(std::move(edge)); // This should now work
//            }
//        }
//
//        for (const auto& edge : edges){
//            g->addNeuronEdge(edge.get());
//        }
//
//    }
//
...
//        // init Graph
//        for (size_t i = 0; i < OrderedNodeName.size() ; ++i) {
//            const auto& currentName = OrderedNodeName[i];
//            SVF::NeuronNode* currentNode = getNodeInstanceByName(currentName);
//            g->addNeuronNode(currentNode);
//        }
//    }
//
//    void Traversal()  {
...
//        SVF::NeuronNode* FirstNode = getNodeInstanceByName(FirstName);
//        SVF::NeuronNode* LastNode = getNodeInstanceByName(LastName);
//
//        dfs->DFS(visited, path, FirstNode, LastNode);
